...
class EnhancedProductionOOTDTrainer:
    ...
    def save_checkpoint(self, epoch, loss, is_best=False):
        ...
        checkpoint_path = os.path.join(checkpoint_dir, f'masked_checkpoint_epoch_{epoch:03d}.pth')
        torch.save(checkpoint, checkpoint_path)
        logger.info(f"Saved checkpoint: {checkpoint_path}")

        # Delete the previous epoch's checkpoint
        if epoch > 0:
            previous_checkpoint_path = os.path.join(checkpoint_dir, f'masked_checkpoint_epoch_{epoch-1:03d}.pth')
            if os.path.exists(previous_checkpoint_path):
                os.remove(previous_checkpoint_path)
                logger.info(f"Deleted old checkpoint: {previous_checkpoint_path}")

        # Save best checkpoint separately
        if is_best:
            best_path = os.path.join(checkpoint_dir, 'masked_best_model.pth')
            torch.save(checkpoint, best_path)
            logger.info(f"Saved best model checkpoint (loss: {loss:.6f})")
    ...
    def load_checkpoint(self):
        ...
        checkpoints = [f for f in os.listdir(checkpoint_dir) if f.endswith('.pth') and 'best' not in f]
        if not checkpoints:
            logger.info("No checkpoints found, starting from scratch.")
            return

        latest_checkpoint_name = sorted(checkpoints, key=lambda x: int(x.split('_')[-1].split('.')[0]), reverse=True)[0]
        ...
        try:
            ...
            self.global_step = self.start_epoch * len(self.dataloader)
